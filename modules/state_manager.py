...
class StateManager:
    ...
    def _schedule_init(self):
        ...
        schedule.every().hour.do(lambda: self.read_dump(StateType.TEMPORARY, False))
        schedule.every().hour.do(lambda: self.read_dump(StateType.PERMANENT, False))
        schedule.every().hour.do(self.current_temp_state_log)
